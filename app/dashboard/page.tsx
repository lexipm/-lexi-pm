...
function SectionHeader({ title, count, icon }: { title: string; count: number; icon: string }) {
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      {count > 0 && (
        <span className="ml-auto text-white text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: '#1A6B4A' }}>
          {count}
        </span>
      )}
    </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1.5">
              {item.channel && <span className="text-xs text-gray-400 font-mono">#{item.channel}</span>}
              <span className="text-xs text-gray-400">{formatDistanceToNow(new Date(item.created_at), { addSuffix: true })}</span>
              <span className={`text-xs font-semibold ml-auto ${riskColor(item.risk_level)}`}>{item.risk_level} risk</span>
            </div>
          </div>
        </div>
      </button>
        ...
        <div className="border-t border-gray-100 px-4 pb-4 pt-3 space-y-3">
          ...
          {item.draft_response && !item.draft_sent && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              ...
              {!draftApproved ? (
                <button
                  onClick={() => { onApproveDraft(item.id, editedDraft); setDraftApproved(true) }}
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => onClose(item.id)} className="flex-1 py-2 rounded-lg text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors">
              Mark closed
            </button>
          </div>
        </div>
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5">
            {task.channel && <span className="text-xs text-gray-400 font-mono">#{task.channel}</span>}
            <span className="text-xs text-gray-400">from {task.addressed_by}</span>
            <span className="text-xs text-gray-400">{formatDistanceToNow(new Date(task.created_at), { addSuffix: true })}</span>
          </div>
        </div>
        <button onClick={() => onClose(task.id)} className="flex-shrink-0 text-xs font-semibold text-gray-400 hover:text-gray-600 px-2 py-1 rounded-md hover:bg-gray-100">
          Close
        </button>
      </div>
    </div>
...
export default function LexiDashboard() {
  ...
  const handleClose = async (id: string) => {
    await fetch('/api/brief', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status: 'closed' }) })
  ...
  const handleApproveDraft = async (id: string, response: string) => {
    await fetch('/api/drafts', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, action: 'approve', edited_response: response }) })
  }

  const urgent = briefItems.filter(i => ['Urgent', 'Decision Needed', 'Client Risk', 'Engineering Blocker'].includes(i.classification))
  const priorities = briefItems.filter(i => ['Delivery Risk', 'Follow-up Required', 'Commercial Opportunity'].includes(i.classification))
  const fyi = briefItems.filter(i => ['FYI', 'Waiting On Others'].includes(i.classification))
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#F0F4F2' }}>
      <div style={{ backgroundColor: '#0D1F17' }} className="sticky top-0 z-10 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between py-4">
            ...
            <button onClick={fetchAll} className="text-xs font-semibold px-3 py-1.5 rounded-lg" style={{ backgroundColor: '#1A6B4A', color: '#fff' }}>
              Refresh
            </button>
          </div>
          <div className="flex gap-1 pb-3">
            {tabs.map(tab => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-colors ${activeTab === tab.id ? 'text-white' : 'text-gray-400 hover:text-gray-200'}`}
                style={activeTab === tab.id ? { backgroundColor: '#1A6B4A' } : {}}
              >
                {tab.label}
                {tab.count > 0 && <span className={`ml-1 text-xs font-bold ${activeTab === tab.id ? 'text-white/80' : 'text-gray-500'}`}>{tab.count}</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        {loading ? (
          <div className="text-center py-16">
            <div className="w-8 h-8 border-2 rounded-full animate-spin mx-auto mb-3" style={{ borderColor: '#1A6B4A', borderTopColor: 'transparent' }} />
          ...
          <>
              ...
              <div>
                {urgent.length > 0 && (
                  <div className="mb-6">
                    ...
                    {urgent.map(item => <BriefCard key={item.id} item={item} onClose={handleClose} onApproveDraft={handleApproveDraft} />)}
                  </div>
                )}
                {priorities.length > 0 && (
                  <div className="mb-6">
                    <SectionHeader title="Priorities and Follow-ups" count={priorities.length} icon="🟡" />
                    {priorities.map(item => <BriefCard key={item.id} item={item} onClose={handleClose} onApproveDraft={handleApproveDraft} />)}
                  </div>
                )}
            ...
            {activeTab === 'tasks' && (
              <div>
                ...
                {tasks.length > 0 ? tasks.map(task => <TaskCard key={task.id} task={task} onClose={handleCloseTask} />) : (
                  <div className="text-center py-16">
                    <p className="text-4xl mb-3">✅</p>
            ...
            {activeTab === 'drafts' && (
              <div>
                ...
                {drafts.length > 0 ? drafts.map(item => <BriefCard key={item.id} item={item} onClose={handleClose} onApproveDraft={handleApproveDraft} />) : (
                  <div className="text-center py-16">
                    <p className="text-4xl mb-3">📭</p>
            ...
            {activeTab === 'fyi' && (
              <div>
                ...
                {fyi.length > 0 ? fyi.map(item => <BriefCard key={item.id} item={item} onClose={handleClose} onApproveDraft={handleApproveDraft} />) : (
                  <div className="text-center py-16">
                    <p className="text-4xl mb-3">🔇</p>
